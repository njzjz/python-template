...
- Install development dependencies:
  - `pip install pytest pytest-cov ruff mypy build pre-commit --user`
  - Takes ~30-60 seconds. NEVER CANCEL. Set timeout to 180+ seconds.
- Install UV and Nox (may fail in network-restricted environments):
  - `pip install uv nox --user`
  - Takes ~10-30 seconds. NEVER CANCEL. Set timeout to 180+ seconds.
...
- Install pre-commit: `pip install pre-commit --user`
...
- Network connectivity may prevent `nox`, `python -m build`, and some `pip install` commands from working
